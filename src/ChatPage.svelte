    ...
    <div class="flex flex-col h-full p-2 w-full">
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            {#each chat.messages as message}
                <div class={message.role === 'user' ? 'border border-gray-rgb rounded p-2 my-2' : 'text text-gray-800 my-4'}>
                    ...
                    <div class="whitespace-pre-wrap prose leading-none
                          prose-h1:my-2
                            ...
                            {#if part.type === 'tool-invocation'}
                                <div class="bg-gray-50 rounded border border-gray-200 p-2 my-2">
                                    <div class="flex items-center gap-2 mb-1.5">
                                        ...
                                        <span class="font-medium text-gray-700">{part.toolInvocation.toolName}</span>
                                        {#if part.toolInvocation.state === 'result'}
                                            <CheckCircle2Icon size={14} class="text-green-500 ml-auto"/>
                                        {:else if part.toolInvocation.state === 'partial-call'}
                                            <Loader2Icon size={14} class="text-blue-500 ml-auto animate-spin"/>
                                        {/if}
                                    </div>
                                    <div class="text-xs font-mono bg-gray-100 rounded p-1.5 overflow-x-auto">
                            ...
                            <div class="flex flex-wrap gap-2">
                                {#each message.experimental_attachments as attachment}
                                    ...
                                    >
                                        ...
                                        <span class="max-w-[200px] truncate">{getBaseName(attachment.name || '')}</span>
                                    </button>
                                {/each}
                            </div>
                        </div>
            ...
            {#if chat.attachments.length > 0}
                <div class="flex flex-wrap gap-2 mb-2">
                    {#each chat.attachments as attachment}
                        <div class="flex items-center gap-1.5 py-1 px-2 bg-gray-100 rounded text-sm">
                            <FileTextIcon class="size-3.5 text-gray-600"/>
                            <span class="max-w-[200px] truncate">{getBaseName(attachment.file.path)}</span>
                            <button
                                    type="button"

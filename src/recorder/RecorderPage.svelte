...
<div class="h-full flex flex-col">
  ...
  <div class="flex-none">
    <div
      class="m-4 p-4 border border-(--background-modifier-border) rounded-lg bg-(--background-primary-alt) min-h-[120px] max-h-[200px] overflow-y-auto"
      bind:this={scrollContainer}
    >
      {#if recorder.isRecording}
        <div
          class="flex items-center justify-between gap-2 mb-2 text-sm text-(--text-accent)"
        >
      ...
      {:else}
        <div
          class="flex items-center justify-center h-full cursor-pointer text-(--text-muted) hover:text-(--text-normal) transition-colors"
          onmousedown={(e) => {
            e.preventDefault();

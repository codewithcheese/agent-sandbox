...
import { SettingsV3Schema } from "./v3";
...
const CHAT_MODEL_PRICING: Record<string, { inputPrice: number; outputPrice: number }> = {
  ...
  "gpt-4o-mini": { inputPrice: 0.60, outputPrice: 2.40 },
  ...
  "claude-3-5-haiku-20241022": { inputPrice: 0.80, outputPrice: 4 },
  ...
  "gemini-2.5-flash-lite-preview-06-17": { inputPrice: 0.10, outputPrice: 0.40 },
};
...
> = {
  version: 4,
  migrate: (data: z.infer<typeof SettingsV3Schema>): z.infer<typeof SettingsV4Schema> => ({
    ...data,
    version: 4,
    models: _.unionBy(
      data.models.map((model) => {
        ...
        if (model.type === "chat") {
          ...
          if (pricing) {
            return { ...model, inputPrice: pricing.inputPrice, outputPrice: pricing.outputPrice };
          }
